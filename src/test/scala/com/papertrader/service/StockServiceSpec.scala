...
class StockServiceSpec extends CatsEffectSuite with Fixture with Matchers {
  ...
  implicit val alphaVantageStockClient
      : AlphaVantageStockClient[Either[Error, *]] =
    new MockAlphaVantageClient {}

  test(
    "service.getGlobalQuote returns quote when stock client returns quote successfully"
  ) {
    val r =
      StockService.getGlobalQuote(MockAlphaVantageClient.stockClientSuccessCase)
    assertEquals(r, Right(globalQuote))
  }

  test(
    "service.getGlobalQuote returns HttpClientParseError when stock client returns HttpClientParseError"
  ) {
    val r = StockService.getGlobalQuote(
      MockAlphaVantageClient.stockClientHttpClientParseErrorCase
    )
    assertEquals(r, Left(HttpClientParseError))
  }

  test(
    "service.getGlobalQuote returns HttpClientServerError when stock client returns HttpClientServerError"
  ) {
    val r = StockService.getGlobalQuote(
      MockAlphaVantageClient.stockClientHttpClientServerErrorCase
    )
    assertEquals(
      r,
  ...
  test(
    "service.getGlobalQuote returns HttpClientNotFoundError when stock client returns HttpClientNotFoundError"
  ) {
    val r = StockService.getGlobalQuote(
      MockAlphaVantageClient.stockClientHttpClientNotFoundErrorCase
    )
  ...
  test(
    ...
    StockService.viewBasketFunc(userId, Map(userId -> testBasket)) should equal(
      testBasket
    )
  }

  test("service.viewBasketFunc returns empty basket for user when non exists") {
    StockService.viewBasketFunc(
      userId2,
  ...
  test(
    ...
    StockService.addToBasketFunc(Item("nvda", 5), userId, b) should equal(
  ...
  test(
    ...
    StockService.addToBasketFunc(Item("ibm", 5), userId, b) should equal(
  ...
  test(
    ...
    StockService.addToBasketFunc(Item("ibm", 5), userId2, b) should equal(
